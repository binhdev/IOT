...
public class GroupRemoteDataSource implements GroupDataSource.RemoteDataSource {
    ...
    public Single<ListGroupResponse> allGroup() {
        return mApiGroup.allGroup(StringHelper.ConcatString(AppConstants.SCHEMA_BEARER,AppPrefs.getInstance(mContext).getApiAccessToken()));
    }

    @Override
    public Single addGroup(Group group) {
        return mApiGroup.addGroup(StringHelper.ConcatString(AppConstants.SCHEMA_BEARER,AppPrefs.getInstance(mContext).getApiAccessToken()),
                group.getName(), group.getDescription());
    }

    @Override
    public Single editGroup(Group group) {
        return mApiGroup.editGroup(StringHelper.ConcatString(AppConstants.SCHEMA_BEARER,AppPrefs.getInstance(mContext).getApiAccessToken()),
                group.getId(), group.getName());
    }

    @Override
    public Single deleteGroup(int id) {
        return mApiGroup.deleteGroup(StringHelper.ConcatString(AppConstants.SCHEMA_BEARER,AppPrefs.getInstance(mContext).getApiAccessToken()),
                id);
    }
}
